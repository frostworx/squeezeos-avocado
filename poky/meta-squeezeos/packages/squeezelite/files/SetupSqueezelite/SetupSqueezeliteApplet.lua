...
local configFile    = "/etc/squeezelite/config"
local configFileTmp = "/tmp/squeezelite-config.tmp"
local initScript = "/etc/init.d/squeezelite"
local UpdateScript = "/etc/squeezelite/updateconfig"
-- local sudo = "sudo "
local sudo = ""
local current
...
function _writeConfig(self)
	...
	os.execute(sudo .. UpdateScript .. " " .. configFileTmp)

	if self:_slIsEnabled() then
		log:info("restarting squeezelite")
		os.execute(sudo .. initScript .. " restart")
	end
end


function _slIsEnabled(self)
	log:info("status squeezelite")
	return os.execute(sudo .. initScript .. " status") == 0
end


function _slEnable(self, new)
	if new then
		os.execute(sudo .. initScript .. " start")
	else
		os.execute(sudo .. initScript .. " stop")
	end
end
